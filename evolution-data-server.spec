...
Source0  : https://download.gnome.org/sources/evolution-data-server/3.54/evolution-data-server-3.54.3.tar.xz
...
%prep
%setup -q -n evolution-data-server-3.54.3
cd %{_builddir}/evolution-data-server-3.54.3
pushd ..
cp -a evolution-data-server-3.54.3 buildavx2
popd
...
export LANG=C.UTF-8
export SOURCE_DATE_EPOCH=1736207333
mkdir -p clr-build
pushd clr-build
