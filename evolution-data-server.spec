...
Source0  : https://download.gnome.org/sources/evolution-data-server/3.54/evolution-data-server-3.54.1.tar.xz
...
%prep
%setup -q -n evolution-data-server-3.54.1
cd %{_builddir}/evolution-data-server-3.54.1
pushd ..
cp -a evolution-data-server-3.54.1 buildavx2
popd
...
export LANG=C.UTF-8
export SOURCE_DATE_EPOCH=1729263074
mkdir -p clr-build
pushd clr-build
