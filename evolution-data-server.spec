...
Source0  : https://download.gnome.org/sources/evolution-data-server/3.52/evolution-data-server-3.52.4.tar.xz
...
%prep
%setup -q -n evolution-data-server-3.52.4
cd %{_builddir}/evolution-data-server-3.52.4
pushd ..
cp -a evolution-data-server-3.52.4 buildavx2
popd
...
export LANG=C.UTF-8
export SOURCE_DATE_EPOCH=1722621733
mkdir -p clr-build
pushd clr-build
