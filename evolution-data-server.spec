...
Source0  : https://download.gnome.org/sources/evolution-data-server/3.54/evolution-data-server-3.54.0.tar.xz
...
%prep
%setup -q -n evolution-data-server-3.54.0
cd %{_builddir}/evolution-data-server-3.54.0
pushd ..
cp -a evolution-data-server-3.54.0 buildavx2
popd
...
export LANG=C.UTF-8
export SOURCE_DATE_EPOCH=1726757813
mkdir -p clr-build
pushd clr-build
